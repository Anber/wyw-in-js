...
`@wyw-in-js/babel-preset` stays available, but uses a separate Node process for eval. Its options must be
JSON-serializable. Move any functions/RegExp config into a `configFile`.
