...
- The old top-level `evaluate` toggle is replaced by `eval.strategy`.
- CSS rule emission order can differ from v1 for equivalent extracted rule sets.
...
### `evaluate` → `eval.strategy`

The old top-level `evaluate` toggle is folded into `eval.strategy`:
...
The Oxc-backed pipeline can emit equivalent extracted rule sets in a different order from v1. This matters when two
generated WyW rules have the same cascade weight and the project relies on source-order ties.
