...
> Note: WyW packages are ESM-only and require Node.js `>=22`. Use an ESM config file
> (for example `vite.config.mjs`) or dynamic `import()` if your project is still CommonJS.
...
`tsconfig.json` discovery, so TypeScript `baseUrl`/`paths` aliases work without duplicating them in WyW config.

The Vite plugin also forwards simple string aliases from `resolve.alias` into native resolver options. RegExp aliases and
plugin-only virtual modules remain Vite-specific and are resolved only by the bundler fallback.

- `hybrid`: tries `eval.customResolver`, then native resolution, then Vite's resolver. Use this when your evaluated code
  mixes normal package/file imports with Vite aliases, virtual modules, or resolver plugins.
- `native`: uses only `oxc-resolver`; Vite's resolver is not called. Mirror any Vite-only alias in `oxcOptions.resolver.alias`
  or keep using `hybrid`.
...
### Emitting metadata sidecars

To emit source-linked metadata manifests for library-owned tooling, enable `outputMetadata`:
...
export default defineConfig(() => ({
  // ...
  plugins: [wyw({ outputMetadata: true })],
}));
```

During build, the Vite plugin emits `.wyw-in-js.json` sidecar assets alongside the normal output. For example, a source module like `src/button.tsx` will produce a metadata asset like `dist/src/button.wyw-in-js.json`.

This path is opt-in and does not affect the default CSS or JS output when disabled.
