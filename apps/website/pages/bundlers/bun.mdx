# Usage with Bun

### Installation

```sh
# npm
npm i -D @wyw-in-js/bun
# yarn
yarn add --dev @wyw-in-js/bun
# pnpm
pnpm add -D @wyw-in-js/bun
# bun
bun add -d @wyw-in-js/bun
```

> Note: WyW packages are ESM-only and require Node.js `>=22` for evaluation (the Bun plugin spawns a Node eval runner).

### Configuration

Bun plugins are configured via the `Bun.build()` API:

```js
import wyw from '@wyw-in-js/bun';

await Bun.build({
  entrypoints: ['src/index.ts'],
  outdir: 'dist',
  plugins: [
    wyw({
      filter: /\.[cm]?[jt]sx?$/,
    }),
  ],
});
```

`eval.require` defaults to `warn-and-run` in v2. If eval-time `require()` happens, WyW will warn and execute it at
runtime; use `importOverrides` or `eval.require: 'error'` to make it explicit.

### Eval resolver modes

`eval.resolver: 'native'` and the native step of `eval.resolver: 'hybrid'` use `oxc-resolver` with automatic
`tsconfig.json` discovery, so TypeScript `baseUrl`/`paths` aliases work by default.

The Bun plugin does not receive a static bundler alias map. Bun-specific resolution is available only through the bundler
fallback.

- `hybrid`: tries `eval.customResolver`, then native resolution, then the Bun/Node fallback resolver used by the plugin.
- `native`: uses only `oxc-resolver`; Bun-specific resolution is not called. Mirror Bun-only aliases in
  `oxcOptions.resolver.alias` or keep using `hybrid`.

### Transforming libraries in `node_modules`

By default, the Bun plugin skips transforming files from `node_modules` for performance.

To transform a specific library, enable `transformLibraries` and narrow `include`/`exclude`:

```js
wyw({
  transformLibraries: true,
  include: [/node_modules\\/(?:@fluentui)\\//],
});
```

### Disabling vendor prefixing

Stylis adds vendor-prefixed CSS by default. To disable it (and reduce CSS size), pass `prefixer: false`:

```js
wyw({
  prefixer: false,
});
```

### Keeping CSS comments

WyW can preserve CSS comments (for example, `/*rtl:ignore*/`) via `keepComments`.

Bun currently strips CSS comments in the final extracted CSS output (including directives like `/*rtl:ignore*/`). Bun may keep top-level `/*!...*/` license comments, but comments inside rules are dropped.

```js
wyw({
  keepComments: true,
  // or keep only matching comments:
  // keepComments: /rtl:/,
});
```
