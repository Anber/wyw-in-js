...
- `hybrid`: tries `eval.customResolver`, then native resolution, then the active Next bundler resolver. Use this when
  evaluated imports may depend on Next, webpack, or Turbopack resolver behavior.
- `native`: uses only `oxc-resolver`; Next's bundler resolver is not called. Mirror bundler-only aliases in
...
- By default, WyW injects `importOverrides` for `react`/`react/jsx-runtime` to avoid resolving Next’s vendored React runtime
  during eval. Override if you need a different runtime.
