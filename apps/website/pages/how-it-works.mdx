...
<Callout emoji="ℹ️">
  The default v2 evaluation path does not execute the module graph in-process anymore. WyW still prepares entrypoints and `__wywPreval`, but actual evaluation now runs in a long-lived Node ESM runner. The host process serves module resolution and loading over `RESOLVE` / `LOAD` RPC, and child modules are prepared lazily when the runner asks for them. The action-level walkthrough below remains useful as compatibility context for the prepare/collect pipeline and older internals.
</Callout>
...
Compatibility note: this section describes the older eager mental model. In the current v2 default path, WyW prepares child modules lazily when the external runner requests them over the broker `LOAD` RPC instead of eagerly walking the whole graph in-process first. In our example, `get-color` is prepared when the runner needs `getColor`.
...
Compatibility note: older implementations created `Module` instances for each entrypoint and evaluated them recursively in-process with [`node:vm`](https://nodejs.org/api/vm.html#vm-executing-javascript). In the v2 default path, `evalFile` sends the prepared root module to the long-lived runner, then the runner asks the host process to `RESOLVE` and `LOAD` additional modules as needed. After the requested modules are evaluated in the runner, we execute the extracted expressions `_exp`, `_exp2`, `_exp3` and `_exp4` from the [Transform Preeval](#transform-preeval) step.
