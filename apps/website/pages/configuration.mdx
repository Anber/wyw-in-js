...
The preset is a compatibility wrapper for environments that still invoke WyW through Babel. Bundler plugins use the Oxc-first transform path directly and do not depend on this preset internally.
...
- When you need those function-valued options, pass only `configFile` inline and let the preset load the rest from that file. That config file may be `.mjs`, but it still must be synchronous.
