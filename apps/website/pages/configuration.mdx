...
- `staticBindings: Record<string, Record<string, unknown>>`
  ...
  A per-source map of statically-known imported values used by `eval.strategy: "hybrid"` and `eval.strategy: "static"`. Keys are either the import
  source as written, such as `@linaria/core`, or the resolved absolute file path. Values map imported names to either
  literal replacements or pure helper functions. Helper functions are called only when all call arguments can also be
  resolved statically.
