...
- `eval: EvalOptionsV2`:
  ...
  - `execute` (default): compute interpolation values with the build-time evaluator.
  - `hybrid`: resolve values that can be proven static first, then fall back to the build-time evaluator.
