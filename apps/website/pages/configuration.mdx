...
- `eval: EvalOptionsV2`:
  ...
  Controls v2 evaluator behavior (`resolver`, `require`, `mode`, `globals`, `onWarn`, and custom resolver/loader hooks).
...
- `staticBindings: Record<string, Record<string, unknown>>`
  ...
  A per-source map of statically-known imported values used by `features.staticImportValues`. Keys are either the import
