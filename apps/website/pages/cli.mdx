...
- `--debug`, `-d`: write debug output into a directory. The log analyzer reads `actions.jsonl`,
  `dependencies.jsonl`, `entrypoints.jsonl`, and optional `eval-files.jsonl` / `perf-spans.jsonl`.
