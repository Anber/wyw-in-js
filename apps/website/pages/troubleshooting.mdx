...
- If you generated WyW debug logs (`actions.jsonl`, `dependencies.jsonl`, `entrypoints.jsonl`, plus optional
  `eval-files.jsonl` / `perf-spans.jsonl`), inspect them with the [log analyzer](/debug/log-analyzer).
...
## Pitfall 4: “Stale” values from `eval.globals` in watch mode

### Typical symptoms

- You changed `eval.globals.FOO` from `1` to `2`, but extracted result still looks like `1`.
- You removed a key from `eval.globals`, but evaluated code still behaves as if it exists.
- Behavior differs between a fresh build and a long-running watch/dev session.

### Current contract in v2

- Changing `eval.globals` values between runs is supported.
- Removing keys from `eval.globals` between runs is supported.
- WyW re-initializes eval runner/cache when serialized globals payload changes.

### Practical checks/workarounds

1) Make sure the value really changes in config input used by WyW:

- If your config is generated dynamically, verify the computed value in logs.
- Prefer deterministic values where possible.

2) Avoid using runtime mutation of globals as your state mechanism:

- Runtime writes inside evaluated modules are not durable across runs.
- Treat `eval.globals` config as the source of truth for each transform.

3) If your toolchain caches config aggressively and changes still do not apply, restart dev/watch process once to
