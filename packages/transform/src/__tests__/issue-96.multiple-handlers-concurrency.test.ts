...
describe('issue #96: actions must not run with multiple handlers', () => {
  ...
  it('allows concurrent transforms when custom handler identity changes', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'wyw-96-'));
    const entryFile = path.join(root, 'main.ts');
    const tokensFile = path.join(root, 'tokens.ts');
    ...
    const createWorkflow = () =>
      function* workflow(this: unknown) {
        return yield* baseHandlers.workflow.call(this as never);
      };

    const run = (workflow: ReturnType<typeof createWorkflow>) =>
      transform(
        {
          cache,
          options: {
    ...
    const first = run(createWorkflow());
    await started.promise;

    const second = run(createWorkflow());
