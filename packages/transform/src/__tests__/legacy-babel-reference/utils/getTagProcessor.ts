...
import type {
  ...
  Params,
  IFileContext,
  SourceLocation as ProcessorSourceLocation,
  TagSource,
...
type BuilderArgs = ConstructorParameters<typeof BaseProcessor> extends [
  ...
  TagSource,
  AstService,
  ProcessorSourceLocation | null,
  (replacement: ProcessorExpression, isPure: boolean) => void,
  ...infer T,
]
  ? T
  : never;
...
function getBuilderForIdentifier(
  ...
  const replacer = (
    replacement:
      | ProcessorExpression
      | ((tagPath: NodePath) => ProcessorExpression),
    isPure: boolean
  ) => {
    mutate(prev, (p) => {
      const next =
      ...
      p.replaceWith(next as Expression);
  ...
  type BabelAstService = typeof t & typeof importHelpers;

  const astService = new Proxy<BabelAstService>(t as BabelAstService, {
  ...
  return (...args: BuilderArgs) =>
    new Processor(
      params,
      tagSource,
      astService as unknown as AstService,
