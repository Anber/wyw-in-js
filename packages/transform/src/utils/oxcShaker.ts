...
type PatternInitializerResolver = (name: string) => Node | null | undefined;
...
const isReceiverOperationProvenInert = (
  operation: ReceiverOperation,
  context: ReceiverProofContext
): boolean => {
...
function isProvenNonAbruptPatternEvaluation(
  pattern: Node,
  value: Node,
  resolveInitializer: PatternInitializerResolver,
...
export const shakeOxcToESM = (
  ...
  statements.forEach((statement) => {
    ...
    const invocation = collectModuleInvocationEffects(
      ...
      (operation) =>
        isReceiverOperationProvenInert(operation, {
