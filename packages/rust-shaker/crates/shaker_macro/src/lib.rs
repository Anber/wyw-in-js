...
fn get_actions(key: &String, cfg: &Yaml) -> proc_macro2::TokenStream {
  let skip = &cfg["skip"];
  ...
  match cfg {
    Yaml::Array(actions) => {
      let mut actions_stream = proc_macro2::TokenStream::new();
      for action in actions {
        actions_stream.extend(get_action_stmt(key, action));
      }
